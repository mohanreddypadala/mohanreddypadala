...
const AboutSection: React.FC<AboutSectionProps> = ({ isDarkMode }) => {
  return (
    <section id="about" className={`py-20 ${isDarkMode ? 'bg-[#0a0a0a] text-gray-300' : 'bg-white text-gray-700'} relative overflow-hidden transition-colors duration-500`}>
      {/* Background Elements */}
      <div className={`absolute top-10 right-10 w-32 h-32 ${isDarkMode ? 'bg-gradient-to-r from-cyan-400 to-purple-500' : 'bg-gradient-to-r from-gray-400 to-gray-600'} rounded-full opacity-5 animate-float`}></div>
      
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center animate-slide-in-left">
            <span className={`${isDarkMode ? 'text-cyan-400' : 'text-gray-600'} font-mono mr-2`}>01.</span>
            <User className={`mr-3 h-6 w-6 ${isDarkMode ? 'text-cyan-400' : 'text-gray-600'}`} />
            About Me
            <div className={`ml-4 h-px ${isDarkMode ? 'bg-gradient-to-r from-cyan-400 to-transparent' : 'bg-gradient-to-r from-gray-600 to-transparent'} flex-grow`}></div>
          </h2>
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-4 animate-fade-in-up">
              ...
              <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} leading-relaxed`}>
                Currently, I serve as the <span className={`${isDarkMode ? 'text-cyan-400' : 'text-gray-800'} font-semibold`}>Lead of Social Media, Design and Marketing</span> at 
              ...
              <div className="flex items-center space-x-4 pt-4">
                <div className={`flex items-center ${isDarkMode ? 'text-cyan-400' : 'text-gray-700'}`}>
            ...
            <div className="md:col-span-1 animate-slide-in-right">
              <div className="relative group">
                <div className={`absolute -inset-2 ${isDarkMode ? 'bg-gradient-to-r from-cyan-400 to-purple-500' : 'bg-gradient-to-r from-gray-400 to-gray-600'} rounded-md opacity-20 group-hover:opacity-40 blur transition duration-500 animate-glow`}></div>
                <div className="relative">
                  <img 
                    ...
                    className="w-full aspect-square object-cover rounded-md hover-lift"
                  />
                  <div className={`absolute inset-0 border-2 ${isDarkMode ? 'border-cyan-400' : 'border-gray-600'} rounded-md -translate-x-3 -translate-y-3 transition group-hover:translate-x-1 group-hover:translate-y-1 duration-300`}></div>
                </div>
              </div>
            </div>
          </div>
