  ...
  const achievements: Achievement[] = [
    {
      ...
      icon: <Star className={`h-8 w-8 ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`} />,
      date: "2024"
    },
    {
      id: 2,
      ...
      icon: <Users className={`h-8 w-8 ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`} />,
      date: "2020-2024"
    },
    {
      id: 3,
      ...
      icon: <Calendar className={`h-8 w-8 ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`} />,
      date: "2020-2024"
    },
    {
      id: 4,
      ...
      icon: <Award className={`h-8 w-8 ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`} />,
      date: "2024"
    },
    {
      id: 5,
      ...
      icon: <Github className={`h-8 w-8 ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`} />,
      date: "2023"
    }
  ];

  return (
    <section id="achievements" className={`py-20 ${isDarkMode ? 'bg-slate-900 text-slate-300' : 'bg-slate-50 text-slate-700'} transition-colors duration-500`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center">
            <span className={`${isDarkMode ? 'text-indigo-400' : 'text-slate-600'} font-mono mr-2`}>04.</span>
            Achievements
          ...
          <div className="space-y-12">
            {achievements.map((achievement) => (
              <div 
                key={achievement.id}
                className={`relative ${isDarkMode ? 'bg-black border-indigo-500' : 'bg-white border-slate-400'} rounded-lg p-6 shadow-lg border-l-4 hover-lift`}
              >
                <div className={`absolute -left-4 -top-4 ${isDarkMode ? 'bg-slate-900 border-black' : 'bg-slate-50 border-white'} p-3 rounded-full border-4`}>
                  {achievement.icon}
                </div>
                
                <div className="ml-4">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{achievement.title}</h3>
                    <span className={`text-sm font-mono ${isDarkMode ? 'text-indigo-400' : 'text-slate-600'}`}>{achievement.date}</span>
                  </div>
                  <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{achievement.description}</p>
                </div>
              </div>
            ))}
          </div>
